let glossary = [
  ...
  function fillCheckBox() {
   ...
   let filteredTags = filterTags.filter((value, index, self) => self.indexOf(value) === index)

   let tagsCheckbox = "";
   filteredTags.forEach((tag) => {
    tagsCheckbox += `<input type="checkbox" id="tags" name="cb" value="${tag}"><label for="${tag}">${tag}</label>`;
   });
   
   div.innerHTML = tagsCheckbox;
  }
 ...
 function clickSearch() {
  let defValue = document.querySelector('input').value.toUpperCase()
  let clasValue = document.querySelector('select').value
  let markedCheckbox = document.getElementsByName('cb') 
  let table = document.querySelector("table"); 
  let tableVal = ''
  let arrMarkedCheckbox = []

  for (var checkbox of markedCheckbox) {  
    if (checkbox.checked)  
    //console.log(checkbox.value)
     arrMarkedCheckbox.push(checkbox.value.toUpperCase());
    }

  tableVal = '<table><tr><th>Term</th><th>Class</th><th>Definition</th><th>Tag</th></tr>'
  tableVal += performSearch(defValue,clasValue,arrMarkedCheckbox)
  tableVal += '</table>'
 ...
 function performSearch(defVal,classVal,tagVal) {
  let rowTable = ""
  
  glossary
  .filter((item) => {
     return  item.definition.toUpperCase().includes(defVal)
  })
  .filter((item) => {
    return item.class.toString().includes(classVal)
  })  
  .filter((item) => {
    return item.tags.toString().toUpperCase().includes(tagVal)
  })  
  .forEach((item) => {
    rowTable += `<tr><td>${item.term}</td><td>${item.class}</td><td>${item.definition}</td><td>${item.tags}</td></tr>`;
  ...
  function handleOnChange(event) {
    ...
    .forEach((item) => {
        ...
        //p.innerHTML = `Term: ${item.term} <br>Class: ${item.class} <br>Description: ${item.definition} <br>Tags: ${item.tags}`
        //div.appendChild(p)
        div.innerHTML = `Term: ${item.term} <br>Class: ${item.class} <br>Description: ${item.definition} <br>Tags: ${item.tags}`
    })
  }
